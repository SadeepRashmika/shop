...
</html>`,b=document.getElementById(`print-receipt-frame`);b&&b.remove();let x=document.createElement(`iframe`);x.id=`print-receipt-frame`,x.style.position=`fixed`,x.style.right=`0`,x.style.bottom=`0`,x.style.width=`0px`,x.style.height=`0px`,x.style.border=`none`,x.style.visibility=`hidden`,document.body.appendChild(x);let S=x.contentWindow.document;S.open(),S.write(y),S.close();let C=()=>{try{x.contentWindow.focus(),x.contentWindow.print()}catch(e){console.error(`Print failed: `,e)}};try{x.contentWindow.document.fonts&&x.contentWindow.document.fonts.ready?x.contentWindow.document.fonts.ready.then(()=>{setTimeout(C,120)}).catch(()=>{setTimeout(C,200)}):setTimeout(C,200)}catch{setTimeout(C,200)}}export{i as n,n as r,r as t};